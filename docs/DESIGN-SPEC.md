...
- Zentriert, flex-column gap 40. Hintergrund-Glow: Kreis 775 px, zentriert, top −130, `blur(32px)`, radialer Verlauf lime 10 % → transparent. Der Kreis läuft oben (130 px + Blur) in die Founder-Sektion und unten in den Footer aus, statt an der Sektionskante beschnitten zu werden (Abweichung 13).
...
13. **CTA (Abschnitt 8) – Glow läuft über die Sektion hinaus.** Der Figma-Frame schneidet den 775-px-Kreis an der Sektionskante ab (Kreis-Oberkante 130 px über der Sektion), auf der Seite stand dadurch eine harte Kante über dem Eyebrow. Umgesetzt (Kundenwunsch, 2026-09-14): die Sektion beschneidet nur noch horizontal (`overflow-x: clip`) und bildet keinen Stapelkontext mehr; der Glow (`z-index: -1`) liegt damit im Wurzel-Stapelkontext hinter dem Inhalt aller Sektionen und über dem Night-Hintergrund, sodass er weich in die Founder-Sektion und den Footer ausläuft.

...
- Beim Scrollen (> 40 px): Hintergrund `rgba(42,50,51,.85)` + `backdrop-filter: blur(12px)`, Übergang 300 ms (Zustandsklasse `.is-scrolled`, gilt auch ohne Scroll-Kopplung). Der Balken bleibt bei top 20, der Hintergrund reicht bis zur Viewport-Oberkante (Abweichung 12).

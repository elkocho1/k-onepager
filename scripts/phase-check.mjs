 ...
 * runs Lighthouse (accessibility category) per page and viewport – desktop
 * 1920 px and mobile 390 px via screen emulation – and stores the full-page
 * screenshot as docs/screens/phase-<n>-<slug>-<viewport>.png next to the
 * JSON report. Needs Chrome or Edge (CHROME_PATH overrides the lookup).
 *
...
/** Runs Lighthouse; returns { score, failing, screenshot } or { error }. */
function lighthouse(url, viewport, reportFile) {
  ...
  return { score, failing, screenshot };
}
...
try {
  for (const page of pages) {
    for (const viewport of VIEWPORTS) {
      ...
      const ok = result.score >= MIN_SCORE;
      failed ||= !ok;
      console.log(`${ok ? 'OK  ' : 'WARN'} ${label} a11y ${String(result.score).padStart(3)}  ${shot}`);
